...
exports.getAllPeople = (req,res,next) => {
   ...
   Peoples.then( ([rows, fieldData]) => {
       //console.log(rows)
        res.render('peoples', { people: rows, peoplesCSS: true, add_artist:true });
   });
...
exports.getPeople = (req,res,next) => {
    ...
    People.then( ([data,metadata]) => {
        res.render('people', {people: data[0], peopleCSS: true, add_artist:false});
    });
}
...
exports.deletePeople = (req,res,next) => {
    peopleModel.deletepeople(req.params.id);
    res.redirect(301, '/peoples');
}